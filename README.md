...
| F13 | 人口密度点查询 | 点击地图任意位置，弹窗显示该点所属网格的人口密度值 |
...
左侧 280px 侧边栏采用**手风琴（accordion）布局**，三个分区——「图层控制」「分析配置」「供需统计」——各自可独立折叠/展开。「图层控制」包含 6 个图层开关；「分析配置」包含设施类型选择、服务半径滑块和盲区阈值；「供需统计」显示覆盖率和详情。底部显示人口密度点查询提示。
...
| GET | `/population/at-point` | 人口密度点查询（`lng`,`lat`），返回指定坐标所属网格的人口密度 |
