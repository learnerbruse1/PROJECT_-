...
### Changed
- **侧边栏改为手风琴布局**：三个可折叠分区——「👁 图层控制」「📊 分析配置」「📈 供需统计」，纵向堆叠，各分区可独立展开/收起，替代原有 Tab 切换模式。
- 侧边栏宽度由 272px 调整为 **280px**。
- 顶栏高度由 42px 调整为 **52px**。
- 恢复顶栏副标题「人口密度 · 设施覆盖 · 供给盲区识别」。
- 恢复 InfoPanel（点选详情/数据说明），以浮层形式置于地图右上角。

### Added
- 供需统计面板（StatsPanel）从侧栏底部移入手风琴分区，作为独立可折叠区块。
