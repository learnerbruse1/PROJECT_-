...
const props = defineProps({
  mode: { type: String, required: true },  // 'layers' | 'analysis'
})
...
function setType(t) { store.analysisType = t }
function resetRadius() { store.resetBufferRadius(store.analysisType) }
function resetAllRadii() { store.resetAllBufferRadii() }
</script>
...
<template>
  <div class="control-panel">
    ...
    <!-- Tab: 分析配置 -->
    <template v-if="props.mode === 'analysis'">
      <section class="block">
        <h3>分析设施类型</h3>
      ...
      <section class="block">
        ...
        <div class="row-between">
          <input class="num" type="number" min="0" step="100" v-model.number="store.popThreshold" />
...
.all-radius { margin-bottom: 0; }
.compact-head { margin-bottom: 3px; }
.compact-hint { margin: 0; }
.radius-row + .radius-row { margin-top: 5px; }
.radius-line {
  display: flex;
  align-items: center;
  justify-content: space-between;
...
.radius-label { font-size: 11px; color: #374151; font-weight: 600; }
.radius-value { font-size: 11px; color: #111827; font-weight: 700; }
.compact-range { margin-top: 3px !important; }
.num {
  width: 110px;
  padding: 6px 8px;
