...
async function refresh() {
  ...
  try {
    ...
    const data = await api.blindSpots(store.bbox, t, store.radiusOf(t), store.popThreshold)
    ...
    layer = L.geoJSON(fc, {
      style: { color: '#b91c1c', weight: 2, fillColor: '#ef4444', fillOpacity: 0.35 },
...
watch(
  () => [store.layers.blindzone, store.analysisType, store.bufferRadius[store.analysisType], store.popThreshold, store.bbox],
  refresh,
)
